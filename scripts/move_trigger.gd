...
func activate():
	...
	for node in get_tree().get_nodes_in_group(targetID):
		...
		if lock_to_player_x and lock_to_player_y:
			var prev = [Global.player.global_position.x, Global.player.global_position.y]
			t.tween_method(func(_p: float):
				var dpx = Global.player.global_position.x - prev[0]
				var dpy = Global.player.global_position.y - prev[1]
				node.global_position.x += dpx * x_move
				node.global_position.y += dpy * y_move
				prev[0] = Global.player.global_position.x
				prev[1] = Global.player.global_position.y,
				0.0, 1.0, duration)

		elif lock_to_player_x:
			var prev = [Global.player.global_position.x]
			...
			var prev = [Global.player.global_position.y]
			var start_x = node.global_position.x
			var target_x = start_x + x_move * 6.4
			t.tween_method(func(p: float):
				node.global_position.x = lerp(start_x, target_x, p)
				var dpy = Global.player.global_position.y - prev[0]
				node.global_position.y += dpy * y_move
				prev[0] = Global.player.global_position.y,
				0.0, 1.0, duration)
